/**
 * @file web_server.c
 * @brief ULTIMATE Web Server v2 - Captive Portal + Timer Control
 * 
 * NEW FEATURES:
 * - Captive Portal (auto-redirect to config page)
 * - Timer Pause/Resume functionality
 * - Info display (button hold time, WiFi duration)
...
static esp_err_t captive_portal_handler(httpd_req_t *req)
{
    ...
    if (host_len > 0) {
        ...
        // If not our IP, redirect
        if (host && strcmp(host, "192.168.4.1") != 0) {
...
esp_err_t web_server_start(void)
{
    ...
    config.max_open_sockets = 15;  // Increased from 7 to 13 for all handlers
    config.lru_purge_enable = true;
    
    ESP_LOGI(TAG, "Starting web server on port %d", config.server_port);
    
    if (httpd_start(&server, &config) == ESP_OK) {
        // Register specific handlers FIRST (before catch-all)
        ...
        // Captive Portal - Register catch-all handler LAST (as fallback)
        ...
        ESP_LOGI(TAG, "Captive Portal enabled - auto-redirect active");
