...
extern StatsForConn* get_conn_entry(uint16_t conn_id);
extern size_t stats_len;
extern StatsForConn stats[];
...
static esp_err_t api_stats_handler(httpd_req_t *req)
{
    ...
    for (size_t i = 0; i < stats_len; i++) {
        ...
        cJSON_AddNumberToObject(device, "conn_id", stats[i].conn_id);
        
        cJSON *stats_obj = cJSON_CreateObject();
        cJSON_AddNumberToObject(stats_obj, "caught", stats[i].stats.caught);
        cJSON_AddNumberToObject(stats_obj, "fled", stats[i].stats.fled);
        cJSON_AddNumberToObject(stats_obj, "spin", stats[i].stats.spin);
